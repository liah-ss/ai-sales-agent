...
| 部分 | 是否包含 | 说明 |
| --- | --- | --- |
| 多语言官网 + 内容管理系统 | ✅ 完整代码 | 目录、内容、询盘、文件、分析、鉴权与缓存 |
| 共享业务 API 层 | ✅ 完整代码 | FastAPI 服务、数据模型、缓存与外部集成 |
| 智能体服务层与前端挂件 | ❌ 未包含 | 商业资产，不对外提供 |
| 业务数据与品牌素材 | ❌ 未包含 | 仓库内示例数据均为通用演示数据 |
| 部署脚本与运维配置 | ❌ 未包含 | 与运行环境强绑定，不对外提供 |

> 仓库中的产品、方案、资讯等内容全部为**通用演示数据**，与任何真实客户无关。
